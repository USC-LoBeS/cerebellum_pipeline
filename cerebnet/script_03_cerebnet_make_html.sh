#!/bin/bash
#$ -S /bin/bash
#$ -o /scratch/faculty/njahansh/projects/cerebellum_segmentation/cerebnet/logs/UKBB_HTML_GEN -j y
#$ -N Cerebnet_HTMLGEN
#$ -V
...
# USAGE
#==============================================================================
# ./script_03_cerebnet_make_html.sh /scratch/faculty/njahansh/projects/cerebellum_segmentation/cerebnet/UKBB/QC/pngs/ 
# /scratch/faculty/njahansh/projects/cerebellum_segmentation/cerebnet/UKBB/QC/ 
# UKBB 
# /scratch/faculty/njahansh/projects/cerebellum_segmentation/cerebnet/UKBB/QC/UKBB_cerebnet_Cerebellum_Outliers_BySubject.csv 
# /scratch/faculty/njahansh/projects/cerebellum_segmentation/cerebnet/UKBB/QC/bounding_box_failures.txt
...
Output:
    If subjects <= BATCH_SIZE:
      - <output_dir>/<dataset>_cerebnet_Cerebellum_QC_brainslice.html
      - (downloadable) <dataset>_cerebnet_Cerebellum_QC_notes.csv
    If subjects > BATCH_SIZE:
      - <output_dir>/<dataset>_cerebnet_Cerebellum_QC_brainslice_batch{N}.html
      - (downloadable) <dataset>_cerebnet_Cerebellum_QC_notes_batch{N}.csv
...
for (( BATCH_NUM=1; BATCH_NUM<=TOTAL_BATCHES; BATCH_NUM++ )); do
    ...
    if [ $TOTAL_BATCHES -eq 1 ]; then
        OUTPUT_HTML="${OUTPUT_DIR}/${DATASET}_cerebnet_Cerebellum_QC_brainslice.html"
        CSV_FILENAME="${DATASET}_cerebnet_Cerebellum_QC_notes.csv"
        BATCH_LABEL=""
    else
        OUTPUT_HTML="${OUTPUT_DIR}/${DATASET}_cerebnet_Cerebellum_QC_brainslice_batch${BATCH_NUM}.html"
        CSV_FILENAME="${DATASET}_cerebnet_Cerebellum_QC_notes_batch${BATCH_NUM}.csv"
...
if [ $TOTAL_BATCHES -eq 1 ]; then
    echo "  ${DATASET}_cerebnet_Cerebellum_QC_brainslice.html"
else
    for (( B=1; B<=TOTAL_BATCHES; B++ )); do
        echo "  ${DATASET}_cerebnet_Cerebellum_QC_brainslice_batch${B}.html"
    done
fi
echo "============================================================"
exit 0