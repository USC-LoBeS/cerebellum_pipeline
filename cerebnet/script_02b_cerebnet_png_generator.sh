...
IMAGE_SUBPATH="mni_ICBM/*_FS_T1_orig_nu_std_2MNI.nii.gz"
LABEL_SUBPATH="mni_ICBM/cerebellum.CerebNet_std_2MNI.nii.gz"

#==============================================================================
# USAGE
#==============================================================================
### qsub /scratch/faculty/njahansh/projects/cerebellum_segmentation/cerebnet/UKBB/scripts/script_02b_cerebnet_png_generator.sh /scratch/faculty/njahansh/projects/cerebellum_segmentation/cerebnet/UKBB/data/ /scratch/faculty/njahansh/projects/cerebellum_segmentation/cerebnet/UKBB/QC/ /scratch/faculty/njahansh/projects/cerebellum_segmentation/cerebnet/scripts_common/FreeSurferColorLUT.txt /ifs/loni/faculty/njahansh/nerds/siddharth/software/anaconda3/bin/python
function Usage(){
    cat << USAGE
...
Usage:
    ...
    qsub $(basename "$0") <subjects_dir> <output_dir> <colormap_file> <python_path> [skip_level] [padding]
...
class CerebellumQC:
    ...
    def __init__(self, subject, image_path, label_path, output_path, colormap_path, skip_level, bb_log, padding):
        ...
        # Default bounding boxes for cerebellum (for QC logging)
    ...
    def load_colors(self):
        """Load color mapping from colormap file and create matplotlib colormap."""
        try:
            ...
            # Parse lines: Index Label_Name R G B A
            parsed_data = []
            max_index = 0
            for l in lines:
                parts = l.split()
                if len(parts) >= 6:
                    idx = int(parts[0])
                    r, g, b, a = map(float, parts[2:6])
                    parsed_data.append([idx, r, g, b, a])
                    max_index = max(max_index, idx)
            
            if not parsed_data:
                raise ValueError("No valid color entries found in colormap file")
            
            # Create color lookup table - ensure all values are in 0-1 range
            self.color_lut = np.zeros((max_index + 1, 4), dtype=np.float32)
            
            for idx, r, g, b, a in parsed_data:
                # Normalize RGB to 0-1 range (they come in as 0-255)
                # IMPORTANT: FreeSurfer colormaps have alpha=0 for all labels
                # We need to set alpha=1 for non-background labels so they're visible!
                if int(idx) == 0:
                    # Background: keep transparent
                    self.color_lut[int(idx)] = [0, 0, 0, 0]
                else:
                    # All other labels: make visible with alpha=1
                    self.color_lut[int(idx)] = [
                        np.clip(r/255.0, 0, 1), 
                        np.clip(g/255.0, 0, 1), 
                        np.clip(b/255.0, 0, 1), 
                        1.0  # Force alpha=1 for visibility
                    ]
            
            # Verify all values are in range
            if np.any(self.color_lut < 0) or np.any(self.color_lut > 1):
                print(f"WARNING: Color values out of range detected!")
                print(f"Min: {self.color_lut.min()}, Max: {self.color_lut.max()}")
                # Force clip all values
                self.color_lut = np.clip(self.color_lut, 0, 1)
            
            # Create matplotlib colormap
            self.cmap = ListedColormap(self.color_lut)
            
            print(f"Loaded {len(parsed_data)} color mappings")
            print(f"Color LUT shape: {self.color_lut.shape}")
            print(f"Color value range: [{self.color_lut.min():.3f}, {self.color_lut.max():.3f}]")
            print(f"Non-zero alpha values: {np.sum(self.color_lut[:, 3] > 0)}")
    ...
    def analyze_label_matching(self):
        ...
        if matched:
            ...
            for idx in matched_sorted[:20]:  # Show top 20
            ...
            if len(matched_sorted) > 20:
                print(f"   ... and {len(matched_sorted) - 20} more")
    ...
    def track_segmentation_voxels(self):
        """Track all non-zero voxel coordinates."""
        inds = np.where(self.label != 0)
        self.nifti_real = list(zip(inds[0], inds[1], inds[2]))
        print(f"Tracked {len(self.nifti_real)} non-zero voxels")
    ...
    def create_image(self, slice_range, orientation, slice_num):
        """Create and save a single slice visualization with thin outlines."""
        try:
            fig, ax = plt.subplots(figsize=(10, 10))
            
            # Get the slices
            bg_slice = np.rot90(self.bg_image[slice_range])
            label_slice = np.rot90(self.label[slice_range])
            outline_slice = np.rot90(self.label_image_erosion[slice_range])
            
            # Show background in grayscale
            ax.imshow(bg_slice, cmap='gray', alpha=1.0)
            
            # Create a masked array for the colored labels (hide background)
            label_masked = np.ma.masked_where(label_slice == 0, label_slice)
            
            # Show colored segmentation overlay with transparency
            # This allows good visibility of underlying brain structure
            im = ax.imshow(label_masked, cmap=self.cmap, alpha=0.20, 
                          vmin=0, vmax=len(self.color_lut)-1, interpolation='nearest')
            
            # Show thin outline using the same segmentation colors but with higher opacity
            outline_masked = np.ma.masked_where(outline_slice == 0, outline_slice)
            ax.imshow(outline_masked, cmap=self.cmap, alpha=0.25, 
                     vmin=0, vmax=len(self.color_lut)-1, interpolation='nearest')
    ...
    def load_data(self):
        ...
        try:
            ...
            label_nib = nib.load(self.label_path)
            self.label = label_nib.get_fdata().squeeze()
            ...
            print(f"Label range: {self.label.min()} to {self.label.max()}")
            
            # Check if labels are within colormap range
            max_label = int(self.label.max())
            if max_label >= len(self.color_lut):
                print(f"WARNING: Label {max_label} exceeds colormap size {len(self.color_lut)}")
    ...
    def process_labels(self):
        """Apply thin erosion to get outlines that preserve label identity."""
        try:
            print("\nProcessing labels for thin outlines...")
            
            # Create thin outlines while preserving label identity
            self.label_image_erosion = np.zeros_like(self.label)
            unique_labels = np.unique(self.label)
            ...
            for i, l in enumerate(unique_labels):
                if l == 0:
                    continue
                mask = self.label == l
                # Use only 1 iteration for thinner outlines
                erosion = binary_erosion(mask, iterations=1)
                # Outline is mask - erosion (creates a 1-pixel border)
                outline = mask & ~erosion
                # Assign the original label value to the outline (not just 1)
                self.label_image_erosion[outline] = l
                
                if (i + 1) % 10 == 0:
                    print(f"  Processed {i + 1}/{len(unique_labels)} labels")
            
            print(f"Thin outline generation complete for {len(unique_labels)} labels")
            print(f"Outline voxels: {np.sum(self.label_image_erosion > 0)}")
            
            # Track segmentation voxels
            self.track_segmentation_voxels()
    ...
    def generate_images(self):
        ...
        try:
            ...
            # Check bounding boxes (for QC logging)
